...
for c in "$@"; do
  ...
  echo "== $(basename "$c")  lines=$(wc -l < "$s") HGMMA=$(grep -c HGMMA "$s") IGMMA=$(grep -c IGMMA "$s")"
  echo "   SETMAXREG: $(grep -n -E 'SETMAXREG' "$s" | sed -E 's/^([0-9]+):.*SETMAXREG[^ ]* *(.*);.*/\1(\2)/' | tr '\n' ' ')"
  echo "   GMMA lines: $(grep -n -E 'HGMMA|IGMMA' "$s" | cut -d: -f1 | sed -n '1p;$p' | tr '\n' ' ')"
  echo "   STL/LDL lines: $(grep -n -E ' (STL|LDL)' "$s" | cut -d: -f1 | tr '\n' ' ')"
  # spill lines between the first and last GMMA => in/around the math loop
  first=$(grep -n -E 'HGMMA|IGMMA' "$s" | head -1 | cut -d: -f1); last=$(grep -n -E 'HGMMA|IGMMA' "$s" | tail -1 | cut -d: -f1)
  if [ -n "$first" ]; then
