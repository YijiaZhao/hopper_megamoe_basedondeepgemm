...
$RUN capture "$R/base_p23" 2 "2 4 8 16"; log "base_p23 done"
...
$RUN capture "$R/m16_base" 3 16 "${FUSED_ONLY[@]}"; log "m16_base done"
$RUN capture "$R/m16_l1all" 3 16 DG_FP4_SPLITK_L1_ALL=1 "${FUSED_ONLY[@]}"; log "m16_l1all done"
$RUN capture "$R/m16_l2all" 3 16 DG_FP4_SPLITK_L2_ALL=1 "${FUSED_ONLY[@]}"; log "m16_l2all done"
$RUN capture "$R/m16_both" 3 16 DG_FP4_SPLITK_L1_ALL=1 DG_FP4_SPLITK_L2_ALL=1 "${FUSED_ONLY[@]}"; log "m16_both done"
