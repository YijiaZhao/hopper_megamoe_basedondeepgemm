...
列口径:MegaMoE int4-SX/mxfp4 = 真8卡含融合通信；FI GG = 单卡 experts=16 shard 纯 GEMM 无通信；
FI 默认=库默认 tactic（小M最快），FI autotune=显式AutoTuner（小M反而慢，见 6.1）；
2×AR = SGLang custom allreduce ×2；+2AR 两列 = FI GG 分别加通信后与 MegaMoE 的公平对比口径。
