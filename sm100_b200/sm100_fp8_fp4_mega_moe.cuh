...
sm100_fp8_fp4_mega_moe_impl(void* y,
    ...
    constexpr auto fp8_intermediate_sf_layout = layout::Data(kIntermediateHidden / 32, false);  // per-token SF bytes need not be 16B (e.g. I=1280 -> 40 B); the buffer is addressed as [k_uint][tokens]
    ...
    // 128 B swizzle atoms along K: an FP8 row of BLOCK_K = 256 spans 2 atoms per stage (A: 8-bit, B: 8-bit containers)
    constexpr uint32_t ATOM_K = 128;
    constexpr uint32_t kNumKAtoms = BLOCK_K / ATOM_K;
    // SF words (4 UE8M0 per 32-bit word, K = 128 per word) per BLOCK_K
    constexpr uint32_t kNumSFWordsPerBlockK = BLOCK_K / 128;

    // Swizzle configs
    constexpr uint32_t kSwizzleAMode = ATOM_K * sizeof(a_dtype_t);
    constexpr uint32_t kSwizzleBMode = ATOM_K * sizeof(b_dtype_t);
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    ...
                    if (is_leader_cta) {
                        // NOTES: the 16U4-typed B load counts its packed global bytes (one tile in total for both CTAs)
        ...
        if (is_leader_cta) {
            ...
            // NOTES: descriptors describe one 128 B swizzle atom (ATOM_K columns); the k-steps of a stage advance
            //        across atoms (atom stride = rows x 128 B) and within an atom (UMMA_K x element bytes)
            auto a_desc = mma::sm100::make_umma_desc<cute::UMMA::Major::K, LOAD_BLOCK_M, ATOM_K, kSwizzleAMode>(smem_a[0], 0, 0);
            auto b_desc = mma::sm100::make_umma_desc<cute::UMMA::Major::K, LOAD_BLOCK_N, ATOM_K, kSwizzleBMode>(smem_b[0], 0, 0);
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    if (cute::elect_one_sync()) {
                        // UTCCP copy SFA and SFB to TMEM, one word plane (K = 128) at a time
                        ...
                        // Issue UMMA: k-step k covers K [k * 32, k * 32 + 32): atom k * 32 / 128, SF word plane k / 4, SF ID k % 4
                        #pragma unroll
                        for (uint32_t k = 0; k < BLOCK_K / UMMA_K; ++ k) {
                            const uint32_t sf_plane = (k * UMMA_K) / 128, sf_id = ((k * UMMA_K) % 128) / 32;
                            const uint32_t atom = (k * UMMA_K) / ATOM_K, in_atom = (k * UMMA_K) % ATOM_K;
                            const auto runtime_instr_desc =
                                mma::sm100::make_runtime_instr_desc_with_sf_id(instr_desc, sf_id, sf_id);
                            a_desc.lo = a_desc_base_lo + ((atom * LOAD_BLOCK_M * ATOM_K + in_atom) * static_cast<uint32_t>(sizeof(a_dtype_t))) / 16u;
                            b_desc.lo = b_desc_base_lo + ((atom * LOAD_BLOCK_N * ATOM_K + in_atom) * static_cast<uint32_t>(sizeof(b_dtype_t))) / 16u;
                            ptx::SM100_MMA_MXF8F6F4_2x1SM_SS::fma(
                                b_desc, a_desc, accum_stage_idx * UMMA_N,
                                k_block_idx > 0 or k > 0, runtime_instr_desc,
                                kTmemStartColOfSFB + sf_plane * kNumSFBTmemColsPerPlane,
                                kTmemStartColOfSFA + sf_plane * kNumSFATmemColsPerPlane);
                        }
                    }
                    __syncwarp();
