...
static std::pair<int, int> get_pipeline_config_for_mega_moe(
    ...
    const bool& fp4_acts = false, const int& sf_vec_size = 32) {
    constexpr int kSmemAlignment = 1024;
    // FP4 activations: packed tokens (0.5 B / element), packed A/B smem tiles, packed L1 output, `128 / (4 * vec)` SF words per BLOCK_K
    const int act_bytes_per_128 = fp4_acts ? 64 : 128;
    const int num_sf_words_per_block_k = fp4_acts ? block_k / (sf_vec_size * 4) : block_k / 128;
    ...
    const int smem_send_buffers_size = align(
        static_cast<int>(layout::Buffer(layout::Data(fp4_acts ? hidden / 2 : hidden), num_dispatch_warps, 1).get_num_bytes()),
        kSmemAlignment);
    ...
    const int smem_sfa_per_stage = sf_block_m * 4 * num_sf_words_per_block_k;
    const int smem_sfb_per_stage = sf_block_n * 4 * num_sf_words_per_block_k;

    // Per-stage: A tile + B tile + SFA tile + SFB tile + full/empty barriers
    // NOTES: the FP8 x FP4 kernel expands FP4 weights to 8-bit containers in smem; the FP4 x FP4 kernel keeps both packed
    const int smem_per_stage = (load_block_m + block_n) * block_k * act_bytes_per_128 / 128 + smem_sfa_per_stage + smem_sfb_per_stage + 2 * 8;
...
static MegaMoEConfig get_mega_moe_config(
    ...
    //        packed FP4 x FP4 tiles are 128 bytes per BLOCK_K = 256 row and also use 128B swizzle
    ...
    if (get_env<int>("DG_JIT_DEBUG") or get_env<int>("DG_PRINT_CONFIGS")) {
        const auto key = fmt::format(
            "MegaMoEConfig(num_ranks={}, num_experts={}, hidden={}, intermediate_hidden={}, num_max_tokens_per_rank={}, num_tokens={}, num_topk={}, fp4_acts={}, sf_vec_size={})",
            num_ranks, num_experts, hidden, intermediate_hidden, num_max_tokens_per_rank, num_tokens, num_topk, fp4_acts, sf_vec_size);
