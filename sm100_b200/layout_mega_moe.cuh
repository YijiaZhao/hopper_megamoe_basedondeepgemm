...
struct Workspace {
    ...
    // [88..91] int L2 DONE count (SM100 push kernels: monotonic, remote red.release.sys, +1 per rank per launch,
    //          signalled by the CTA that completes the last local L2 task; replaces the combine NVLink barrier)
    // [92..95] L2 task arrival count (local; the signalling CTA resets it)
    CUTLASS_DEVICE
    int* get_l2_done_count_ptr() const {
        return math::advance_ptr<int>(base, 88u);
    }

    CUTLASS_DEVICE
    uint32_t* get_l2_task_arrival_ptr() const {
        return math::advance_ptr<uint32_t>(base, 92u);
    }
