...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if args.active_ranks:
        num_tokens = num_tokens if rank_idx in [int(v) for v in args.active_ranks.split(',')] else 0
    ...
    def create_inputs():
        ...
        if args.balanced:
            # forced-balanced routing (same rule as the H20 / B200 tables): slot s -> rank s, expert = s*E_local + (g + 7 s) % E_local
            g = (torch.arange(num_tokens, device='cuda', dtype=torch.int64) + rank_idx * 1000003)
    ...
    if args.bench_stream:
        # Streamed replays (B200 / H20 table method): L2 flush -> input copy -> on-stream all-reduce (aligns ranks) -> kernel
        ...
        for i in range(n_warm + n_iter):
            flush_buf.zero_(); copy_inputs_to_buffer(); dist.all_reduce(align_buf, group=group)
        ...
        dist_print(f'STREAM us [{args.mma_type}] per-rank (last3-median): ' + ' '.join(f'{v:.1f}' for v in t_all[:, 0].tolist()) +
        ...
        dist_print(f'KSPAN us [{args.mma_type}] per-rank (last3-median kernel span): ' + ' '.join(f'{v:.1f}' for v in k_all[:, 0].tolist()) +
...
if __name__ == '__main__':
    ...
    parser.add_argument('--active-ranks', type=str, default='', help='ranks that carry tokens; others get 0 tokens')
