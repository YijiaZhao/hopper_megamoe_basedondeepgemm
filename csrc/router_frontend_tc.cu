// Tensor-core fused router + top-k + activation-quantization frontend (SM90),
// shape-generic replacement for the Kernel-Factory frontends (see router_frontend_tc.h). One launch, two CTA roles:
...
// Workspace (ticket counters + up to 4 K-split slices of [m, e] fp32 logits) is
// owned here so the public API matches the existing KF launchers.
static void* tc_workspace(int device) {
    static void* ws[64] = {};
    if (ws[device] == nullptr) {
        cudaMalloc(&ws[device], 256 + 4 * 64 * 512 * sizeof(float));
        cudaMemset(ws[device], 0, 256);
    }
    return ws[device];
}
...
void launch_router_quant_topk_tc(
    ...
    void* workspace = tc_workspace(device);
...
bool router_quant_topk_tc_supported(int m, int h, int e, int topk) {
    return m >= 1 && m <= 64 && h % 1024 == 0 && e % 16 == 0 && e <= 512 && topk >= 1 && topk <= 8;
}
