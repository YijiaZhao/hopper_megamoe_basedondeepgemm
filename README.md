...
| FE swapped-operand router MMA: experts on the MMA M dimension (mma.sync m16n8k16 bf16, tokens pad to 8), weight A fragments ld.global.nc straight into registers (whole K-part in flight, no smem ring / TMA), activations staged once in smem; optional one-time A-fragment weight permutation so one warp load = one contiguous 512 B (`deep_gemm.fable_router_weight_fragment_layout`) | `DG_FE_TINYM_MMA=swapab`, `DG_FE_ROUTER_WLAYOUT=row|fragment|pre` | kept OFF: H20-3e kernel end (stamps, rows 1/2 x mxfp4/qoq) 96x4 wmma 6.91 -> swapab row 6.66 (-0.25 us, < 0.3 us bar) -> swapab+fragment 6.14 (-0.77 us, but the caller must pass fragment-order weights); 78 full-K swapab 6.40-6.91 (vs 7.94-8.45 full-K wmma); NCU 96/rows 1: L2 read requests 49.1k -> 37.7k, LSU inst 89k -> 29k; top-8 sets identical on 54k rows (swapab row: 0 mismatches, 0 weight flips; full-K swapab: 0 mismatches, 16 bf16 flips) |
...
| FE CUDA-core K-split router (full-K grid, m <= 2): CTA = 5 experts x 4 warps (640 threads), one warp = one expert x one quarter of K, all weight chunks `ld.global.nc` straight into registers as the first instructions; last-arriving router CTA (`atom.acq_rel.gpu` ticket) reads the 616 key slots and does top-8 + softmax (no polling merger CTA); the spare CTA quantises the rows | `DG_FE_TINYM_GRID=auto DG_FE_TINYM_MMA=cc` (`cc6` = 6 warps/expert; `DG_FE_CC_MERGE=ticket|poll`, `DG_FE_CC_SELECT=insert|redux`) | kept OFF pending the customer bench: H20-3e kernel end (stamps, rows 1/2 x mxfp4/qoq) 96x4 wmma 6.91/7.17/6.66/6.91 -> cc 4.61/4.61-4.86/4.86/4.86 us (-2.0..-2.6; +`DG_FE_ROUTER_L2_PERSIST=1` 4.35/4.35/4.61/4.61; poll merger 4.86-5.38); remaining tail after the last logits (~2.3): atomic ticket 0.25-0.5, key read 0.25, top-8 select 0.5-0.75, softmax+write 0.25; microkernel (`csrc/router_cc_bench.cu`) all 384 logits at 2.05 us, the SM's outstanding-request capacity bounds the 2.36 MB stream (TMA bulk variants 2.75-3.4 us, slower); top-8 sets identical on 4200 row-evaluations (2 bf16 flips), x/x_sf bit-identical; 8-rank fused correctness cos_min 0.99999 (mxfp4) / 0.99993 (qoq) |
| FE padding-free tensor-core tiny-M router (full-K grid, m <= 2): X[3072] viewed as an A tile 16 x 192 (row r = K-segment r, real data in every row, no M padding); work unit = (K-segment, 16-expert group) = one m16n16k192 `mma.sync m16n8k16` bf16 product whose row `segment` is the segment's 16 logit partials (the other 15 rows are free junk); 384 units over 77 CTAs x 5 units (30 KB weights/CTA, same bytes/SM as cc), A + B fragments `ld.global.nc` straight into registers as the first instructions; 16 segment partials/expert as never-zero order-preserving u32 slots, relaxed ticket, last-arriving CTA sums seg 0..15 in fixed order, bf16-rounds, top-8 + softmax; spare CTA quantises | `DG_FE_TINYM_GRID=auto DG_FE_TINYM_MMA=tc16` (6 warps/unit = 960 thr, 1 k32 block/warp), `tc16w3` (3 warps/unit, 2 blocks), `tc16w2` | kept OFF (slower than cc): H20-3e kernel end (stamps, rows 1/2, mxfp4 = qoq within 0.25) tc16 6.66/9.73, tc16w3 6.14/8.70 vs cc 4.61/4.86, wmma96 6.91/6.91; +`DG_FE_ROUTER_L2_PERSIST=1` tc16 6.91/9.73, tc16w3 5.89/8.45 vs cc 4.35/4.61. Where it loses: weight chunk0 lands 2.3-2.8 us (cc 1.0-1.5; every lane also issues 2m L2-hot A vectors, 30 warps/SM), all partials written 3.6-4.1 (cc 2.8-3.1), and the last CTA's reduction of 16 x 384 x m fp32 partials (24 KB/token vs cc's 2.4 KB of keys) costs 1.3 us (rows 1) / 2.5 us (rows 2) before the select; rows 2 also doubles the MMA work per warp. Equality vs 96x4 WMMA: 6000 row-evaluations each, top-8 sets identical (tc16 2 bf16 weight flips, tc16w3 0), x/x_sf bit-identical; NCU (tc16 rows 1 mxfp4, --set full, clock-control none): 8.70 us, 78 x 960, 64 regs, achieved occupancy 46%, DRAM read 2.42 MB, L2 read requests 70.4k / sectors 140.9k (2.36 MB weights + the 6 KB A tile per unit = 2.3 MB of L2-hot X re-reads), tensor pipe 4.9% active, stalls/issue long_scoreboard 10.3 barrier 7.3 no_instruction 3.3; 8-rank fused correctness (tokens/rank 1, 2, 8) cos_min 0.99999 (mxfp4) / 0.99993 (qoq), unchanged vs cc |
