...
| FE CUDA-core K-split router (full-K grid, m <= 2): CTA = 5 experts x 4 warps (640 threads), one warp = one expert x one quarter of K, all weight chunks `ld.global.nc` straight into registers as the first instructions; last-arriving router CTA (`atom.acq_rel.gpu` ticket) reads the 616 key slots and does top-8 + softmax (no polling merger CTA); the spare CTA quantises the rows | `DG_FE_TINYM_MMA=cc` (opt-in; an unset grid then follows -> `auto`, unset `DG_FE_ROUTER_L2_PERSIST` -> 1); `cc6` = 6 warps/expert, `cc44` = 4 x 4 warps (grid 97); `DG_FE_CC_MERGE=ticket|poll`, `DG_FE_CC_TICKET=relaxed|acqrel`, `DG_FE_CC_SELECT=insert|redux` | kept OFF: the kernel-end stamps are the shortest of all FE variants but the customer-method kernel span is the longest -- same-session A/B at M=8 (nsys span, 1830 MHz, mxfp4/qoq): cc+persist 8.99/8.86, cc 9.41/9.25, wmma 8.00/8.74, swapab+fragment 7.68/7.52 us; three full captures with cc+persist as default: FE 9.0-9.7 us at M=2/8/16 (`/raid/kimi/results/merge/cap_final`, `cap_ab`); H20-3e kernel end (stamps, rows 1/2 x mxfp4/qoq) 96x4 wmma 6.91/7.17/6.66/6.91 -> cc 4.61/4.61-4.86/4.86/4.86 us (-2.0..-2.6; +`DG_FE_ROUTER_L2_PERSIST=1` 4.35/4.35/4.61/4.61; poll merger 4.86-5.38); remaining tail after the last logits (~2.3): atomic ticket 0.25-0.5, key read 0.25, top-8 select 0.5-0.75, softmax+write 0.25; microkernel (`csrc/router_cc_bench.cu`) all 384 logits at 2.05 us, the SM's outstanding-request capacity bounds the 2.36 MB stream (TMA bulk variants 2.75-3.4 us, slower); top-8 sets identical on 4200 row-evaluations (2 bf16 flips), x/x_sf bit-identical; 8-rank fused correctness cos_min 0.99999 (mxfp4) / 0.99993 (qoq) |
...
CUDA-core FE router as the default (`DG_FE_TINYM_MMA=cc`: kernel-end stamps 6.91 -> 4.35 us, but nsys kernel span 8.9-9.7 vs 7.5-7.7 us swapab under the customer method; top-8 sets identical on 54k rows, so it stays a validated opt-in).
