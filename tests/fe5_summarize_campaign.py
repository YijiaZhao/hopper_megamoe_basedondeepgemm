...
for d in sorted(root.glob("*_p*")):
    ...
    for row in json.load(open(js)):
        ...
        if m.group(4):
            ...
            continue
        c = cells.setdefault((m.group(1), m.group(2), row["precision"], row["M"]), {"fe": [], "mega": [], "e2e": []})
...
print("| Precision | M | routing | FE base | FE lean | E2E Mega base | E2E Mega lean | E2E base | E2E lean | Mega-only | passes |")
print("|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|")
for q in ("mxfp4", "qoq"):
    for M in (2, 4, 8, 16):
        for mode in ("normal", "balanced"):
            b = cells.get(("base", mode, q, M), {"fe": [], "mega": [], "e2e": []}); l = cells.get(("lean", mode, q, M), {"fe": [], "mega": [], "e2e": []})
