...
def _run_case(args: argparse.Namespace, m_tokens: int, weight_scale: float,
    ...
    if rank_idx == 0:
        print(
            ...
            f"block_n={selected_block_n}, reference=exact_mxfp4 ===",
            flush=True,
        )
    ...
    for token_idx in range(num_global_tokens):
        for topk_i in range(num_topk):
            ...
            scaled_amax = (intermediate_amax / 448.0).clamp_min(
                torch.finfo(torch.float32).tiny)
            intermediate_sf = torch.exp2(torch.ceil(torch.log2(scaled_amax)))
            intermediate = (
...
def _worker(local_rank: int, num_local_ranks: int, args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if rank_idx == 0:
            ...
            print(
                f"AGGREGATE reference=exact_mxfp4 "
