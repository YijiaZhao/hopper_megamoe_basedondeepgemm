...
configuration per GPU in parallel (CUDA_VISIBLE_DEVICES=k).
  ...
  python3 tests/fe_standalone_bench.py --quant mxfp4 --rows 1
"""
import argparse
import os
import sys
...
def main():
    ...
    path = deep_gemm.fable_frontend_path(args.rows, HIDDEN, EXPERTS, TOPK)
    # DG_FE_ROUTER_WLAYOUT=fragment (swapab path): permute ONCE here (weight-transform time) and pass
    # wlayout="pre", so the timed call has no wrapper-side lookup (the standalone event is CPU-launch-bound).
    wlayout = os.environ.get("DG_FE_ROUTER_WLAYOUT", "fragment")
    if wlayout == "fragment" and path == deep_gemm.mega.FE_PATH_SWAPAB:
        w = deep_gemm.fable_router_weight_fragment_layout(w)
        wlayout = "pre"

    def fe(st=0):
        deep_gemm.fable_router_quant_topk_frontend(x, w, buf, quant=args.quant, stamps=st, wlayout=wlayout)
    ...
    n_router = deep_gemm.fable_frontend_router_ctas(args.rows, EXPERTS, HIDDEN, TOPK)
    cc = path == deep_gemm.mega.FE_PATH_CC
    print(f"== FE standalone eager (L2 flush before each launch): quant={args.quant} rows={args.rows} "
          f"path={path} DG_FE_ROUTER_WLAYOUT={os.environ.get('DG_FE_ROUTER_WLAYOUT', 'fragment')} router CTAs={n_router} cc={int(cc)} "
    ...
    attribution(collected, n_router, args.rows, cc)
