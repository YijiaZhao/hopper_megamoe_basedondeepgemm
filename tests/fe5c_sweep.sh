...
# Usage (repo root, 8 GPUs): LEANS="0 1" SELS="1 0" ROUTINGS="normal balanced" SEEDS="20260903" NSEEDS=1 OUT=/raid/kimi/results/fe5c bash tests/fe5c_sweep.sh
...
for lean in $LEANS; do for sel in $SELS; do for routing in $ROUTINGS; do for seed in $SEEDS; do for shape in $SHAPES; do
  ...
  tag="lean${lean}_sel${sel}_${routing}_seed${seed}x${NSEEDS}_${name}"
  log="$OUT/$tag.log"
