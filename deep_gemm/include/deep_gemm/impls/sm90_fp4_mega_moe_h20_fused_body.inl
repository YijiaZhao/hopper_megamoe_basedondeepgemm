                    ...
                    const auto decode_stage_rf = [&](const uint32_t& stage, const uint32_t& kb,
                        ...
                        for (uint32_t h = 0; h < 2; ++ h) {
                            ...
                            for (uint32_t r = 0; r < 2; ++ r) {
                                #pragma unroll
                                for (uint32_t k = 0; k < 4; ++ k) {
                                    // kE2M1AndE8M0RelToFp8Lut rows s = 4..15 are byte-wise linear in s
                                    // (each +1 in s adds 8 to every E4M3 byte = x2); rows 0..3 hold
                                    // flush/subnormal magnitudes and stay on the SMEM table. This
                                    // removes the dependent LUT gather from the common decode path.
                                    const uint32_t sidx = (sw[h][r] >> (k * 8u)) & 0x7fu;
                                    const uint32_t sm4 = sidx - 4u;
                                    if (kMXFP4 && sm4 < 12u) {
                                        lut[h][r][k] = make_uint2(sm4 * 0x08080800u + 0x14100800u,
                                                                  sm4 * 0x08080808u + 0x24201c18u);
                                    } else {
                                        lut[h][r][k] = smem_nvfp4_lut[sidx];
                                    }
                                }
                            }
                        }
