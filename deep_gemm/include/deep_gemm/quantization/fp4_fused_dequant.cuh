...
//
// NVFP4 (E2M1 + UE4M3 scale) and MXFP4 (E2M1 + E8M0 scale) -> FP8 (E4M3)
// dequant helpers for SM90
...
// MXFP4 (E2M1 + E8M0 scale) variant. The host normalises every weight row by a
// per-row reference exponent `e_ref = e_max_row - 6` and stores the relative
// scale index `s = e - e_ref + 9` (clamped to [0, 15]) in the fused layout.
// Row 0 flushes to zero; row s >= 1 holds magnitudes 0..7 times 2^(s - 9), i.e.
// 2^-8 .. 2^6. Every product is exactly representable in E4M3 (down to the
// subnormal 2^-9), so the decoded FP8 tile is bit-exact and only the per-row
// factor 2^e_ref (times any global scale) must be applied in the epilogue.
static __device__ __constant__ __align__(16) const uint2 kE2M1AndE8M0RelToFp8Lut[128] = {
    {0x00000000u, 0x00000000u},
    {0x03020100u, 0x0c080604u},
    {0x06040200u, 0x14100c08u},
    {0x0c080400u, 0x1c181410u},
...
// Relative scale index bias: LUT row `s` encodes 2^(s - kMXFP4LutExpBias).
static constexpr int kMXFP4LutExpBias = 9;
// Reference exponent margin: e_ref = e_max_row - kMXFP4RefExpMargin keeps
// 6 * 2^(e_max - e_ref) = 384 <= 448 in E4M3.
static constexpr int kMXFP4RefExpMargin = 6;
